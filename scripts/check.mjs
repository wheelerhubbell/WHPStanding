...
const root=resolve(new URL('..',import.meta.url).pathname);process.chdir(root);const evidence='evidence/propagation-extension';await mkdir(evidence,{recursive:true});
...
function run(command,args,options={}){const result=spawnSync(command,args,{encoding:'utf8',maxBuffer:16777216,timeout:300000,...options});if(result.error||result.status!==0)throw Error((result.error?.message??'COMMAND_FAILED')+'\n'+result.stdout+'\n'+result.stderr);return result;}
try{
  ...
  const report={build:'WHP Standing v1',version:'1.0.0-candidate.1+discovery.1',started_at:started,completed_at:new Date().toISOString(),
    ...
    separate_process_retrieval_executed:true,independent_python_verifier_executed:true,test_artifact_refused_as_live:true,cold_agent_test_environment:'TEST',cold_agent_report:JSON.parse(await readFile('evidence/cold-agent/report.json','utf8')),
