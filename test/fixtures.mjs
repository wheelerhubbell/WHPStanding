...
export function fixture(){
  ...
  const bounds={scope,jurisdiction,valid_from:NOW-100,valid_until:NOW+3600};
  const rootPub=publicDer(root.privateKey),rootPin=keyId(rootPub);
  const cert=(k,subject,roles)=>seal('WHP-AUTHORITY-CERTIFICATE-v1',{public_key:publicDer(k.privateKey),subject,roles,scopes:[scope],jurisdictions:[jurisdiction],operations:['INFORM','RECOMMEND','EXECUTE'],profile_hash:PROFILE_HASH,valid_from:NOW-1000,valid_until:NOW+604800},root.privateKey);
  const trustBundle={root_public_key:rootPub,profile_authorization:seal('WHP-PROFILE-AUTHORIZATION-v1',{
    profile_hash:PROFILE_HASH,ratified:true,issuer:'WHP Standing test issuer — not an institutional issuance',environment:'TEST',valid_from:NOW-1000,valid_until:NOW+604800},root.privateKey),
    certificates:[cert(issuer,'test-issuer',['ISSUER','REGISTRY']),cert(source,'test-source-attestor',['SOURCE']),cert(transition,'test-transition-authority',['TRANSITION'])],revocations:[]};
  trustBundle.status_snapshot=seal('WHP-TRUST-STATUS-v1',{sequence:0,previous_hash:null,profile_authorization_hash:hash(trustBundle.profile_authorization),certificates_hash:hash(trustBundle.certificates),revocations_hash:hash(trustBundle.revocations),valid_from:NOW-1000,valid_until:NOW+604800},root.privateKey);
  const leaf={id:'sample-observation',version:'1',content:{statement:'Three items are present in this submitted fixture.',count:3},locator:'urn:whp:test:source-1',epistemic_status:'REPORT',
    ...
    ...bounds,valid_from:NOW-500,valid_until:NOW+7200,status:'ACTIVE',prior_hash:null};
  ...
  const edge={from:[hash(leaf)],to:hash(target),transform:'COPY',operations:['INFORM','RECOMMEND'],...bounds,valid_from:NOW-500,valid_until:NOW+7200,
