...
The normative first-public contract is `contracts/WHP-STANDING-v1.md`. This document supplies the inherited operational rules. The unreleased candidate is development provenance, not another public wire format.

## Canonical representation and signatures

WHP-JCS-I1 uses sorted UTF-16 object keys, UTF-8 output, JSON string escaping, preserved array order, and no Unicode normalization. It deliberately admits only safe integer numeric values; decimal token quantities are strings. Noninteger lexical forms such as `1.0` and `1e0` are rejected even when numerically integral. Negative zero, nonintegral/unsafe numeric values, duplicate object keys, lone surrogates, and excessive nesting are rejected. The general canonicalizer allows depth 64. A submission reserves 16 levels for the final envelope, so its effective maximum is 48. The HTTP body ceiling is 262,144 bytes. Trust bundles are capped at 65,536 bytes. Text admission limits count Unicode scalar values. JSON Schemas describe structure; these additional byte, Unicode, graph and cross-field constraints are normative too.
...
## Portable first-public v1 discovery

The signed `discovery` object binds a self-certifying issuer identity, a root-qualified capability, exact claim pointers, the embedded profile hash and exact independent verifier source commitment. It binds only the stable canonical resolution identity, not a current commercial service origin. The separately signed resolution binds the current provider contract, both schemas, OpenAPI, profile and registry route. The full closed grammar and all required fields are in `schemas/result.schema.json`, `schemas/registry.schema.json` and `schemas/resolution.schema.json`. No semantic extension bag or candidate fallback is permitted.
