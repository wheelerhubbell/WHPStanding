...
export async function runtimeFromEnvironment(env=process.env){
  for(const name of ['WHP_ORIGIN','WHP_DISCOVERY_ORIGIN','WHP_RESOLUTION_SEQUENCE','WHP_ROOT_PIN','WHP_ISSUER_PRIVATE_KEY','WHP_FACILITATOR_URL','WHP_RPC_URL'])demand(env[name],'CONFIG_'+name+'_REQUIRED',503);
  ...
  const privateKey=createPrivateKey(env.WHP_ISSUER_PRIVATE_KEY),rail=new EvmRail({facilitator_url:env.WHP_FACILITATOR_URL,rpc_url:env.WHP_RPC_URL,network:requirements.network,discovery_extensions:{bazaar:bazaar({origin:env.WHP_ORIGIN,discoveryOrigin:env.WHP_DISCOVERY_ORIGIN})}});
  const store=await postgresStore(env.DATABASE_URL??env.NETLIFY_DB_URL);
  try{return new StandingService({store,rail,privateKey,trustBundle,rootPin:env.WHP_ROOT_PIN,origin:env.WHP_ORIGIN,discoveryOrigin:env.WHP_DISCOVERY_ORIGIN,resolutionSequence:Number(env.WHP_RESOLUTION_SEQUENCE),resolutionPreviousHash:env.WHP_RESOLUTION_PREVIOUS_HASH??null,requirements});}catch(e){await store.close();throw e;}
}
